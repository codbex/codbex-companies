angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-companies-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Companies" && e.view === "Company" && (e.type === "page" || e.type === undefined));
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Company", `Unable to list/filter Company: '${response.message}'`);
						return;
					}
		...
		$http.get("/services/ts/codbex-cities/gen/codbex-cities/api/Countries/CountryService.ts").then(function (response) {
